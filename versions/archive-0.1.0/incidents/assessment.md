...
Should the intruder steal an access drive and force declaration of the encryption key, the team may assume that they use that key to access all investor information. The team may, at their discretion, attempt to provide a false key which could leave the attackers in the state of Breach Assessment while team not present.[^false-key-mitigation] However, should this mitigation step be unavailable (for example, due to the thieves checking the provided key and/or data), the team may divulge the access credentials for their own safety.

[^false-key-mitigation]: I guess in the future we could do something more elaborate here as needed, like a fake decryption key that provides limited temporary reserve access to the information, while also somehow relaying a local node to send out a distress signal. But that would be nuanced to set up because you can only allow emergency signal egress, and no ingress. Probably not necessary with a reasonable timeframe on the [migration](https://github.com/blocktransfer/org-docs/milestone/2).
